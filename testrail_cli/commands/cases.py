...
def list_cases(
    ...
    section_id: Optional[int] = typer.Option(None, help="Section ID filter"),
    ...
    updated_after: Optional[str] = typer.Option(
        None, help="Updated after (ISO8601 or epoch)"
    ),
    updated_before: Optional[str] = typer.Option(
        None, help="Updated before (ISO8601 or epoch)"
    ),
    priority_id: Optional[str] = typer.Option(
        None, help="Priority ID(s), comma-separated"
    ),
    type_id: Optional[str] = typer.Option(None, help="Type ID(s), comma-separated"),
...
def add_case(
    ...
    template_id: Optional[int] = typer.Option(None, help="Template ID"),
    type_id: Optional[int] = typer.Option(None, help="Type ID"),
    priority_id: Optional[int] = typer.Option(None, help="Priority ID"),
    estimate: Optional[str] = typer.Option(
        None, help="Estimate (e.g., '30s', '1m', '2h')"
    ),
    refs: Optional[str] = typer.Option(None, help="References (comma-separated)"),
    output: str = typer.Option("json", help="Output format (json, table, raw)"),
) -> None:
...
def update_case(
    ctx: typer.Context,
    case_id: int = typer.Argument(..., help="Case ID"),
    title: Optional[str] = typer.Option(None, help="Case title"),
    template_id: Optional[int] = typer.Option(None, help="Template ID"),
    type_id: Optional[int] = typer.Option(None, help="Type ID"),
    priority_id: Optional[int] = typer.Option(None, help="Priority ID"),
    estimate: Optional[str] = typer.Option(None, help="Estimate"),
    refs: Optional[str] = typer.Option(None, help="References"),
    output: str = typer.Option("json", help="Output format (json, table, raw)"),
) -> None:
    """Update a test case."""
    client: TestRailClient = ctx.obj["client"]
...
def delete_case(
    ctx: typer.Context,
    case_id: int = typer.Argument(..., help="Case ID"),
    soft: Optional[int] = typer.Option(None, help="Soft delete (1) or hard delete (0)"),
    ...
    if not yes:
        delete_type = (
            "soft delete" if soft == 1 else "hard delete" if soft == 0 else "delete"
        )
        confirm = typer.confirm(
            f"Are you sure you want to {delete_type} case {case_id}?"
        )
        if not confirm:
            raise typer.Abort()
...
def import_cases(
    ...
    suite_name: Optional[str] = typer.Option(
        None, help="Suite name (alternative to suite_id)"
    ),
    section_path: Optional[str] = typer.Option(
        None, help="Default section path (e.g., 'Parent/Sub')"
    ),
    mapping: Optional[str] = typer.Option(None, help="Path to mapping YAML/JSON file"),
    create_missing_sections: bool = typer.Option(
        False, help="Create sections if missing"
    ),
